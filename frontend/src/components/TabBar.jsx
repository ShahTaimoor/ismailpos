...
import { X, XSquare, Split, Columns } from 'lucide-react';
      ...
      <div className="flex flex-col">
        <div className="flex items-center justify-between px-2 sm:px-4 py-2 gap-2 min-w-0">
          ...
          <div
            ref={tabListRef}
            role="tablist"
            className="flex items-center space-x-1 overflow-x-auto flex-1 min-w-0 scrollbar-hide"
          >
            {tabs.map((tab) => {
        ...
        {/* Split View Tab Selection - Separate row */}
        {isSplitView && !isMobile && !isTablet && tabs.length >= 2 && (
          <div className="flex items-center gap-2 sm:gap-4 px-4 py-2 bg-gray-50 border-t border-gray-200 text-xs">
            <span className="text-gray-600 shrink-0">Left:</span>
            <select
              value={leftTabId || ''}
              onChange={(e) => setLeftTab(e.target.value)}
              aria-label="Select left tab"
            ...
            <span className="text-gray-600 shrink-0">Right:</span>
            <select
              value={rightTabId || ''}
              onChange={(e) => setRightTab(e.target.value)}
              aria-label="Select right tab"
              className="border border-gray-300 rounded px-2 py-1 text-xs bg-white"
            >
              <option value="">Select tab...</option>
              {tabs.map(tab => (
